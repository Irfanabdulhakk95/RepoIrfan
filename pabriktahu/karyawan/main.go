...
import (
	...
	ep "pabriktahu/karyawan/endpoint"
...
func main() {
	...
	var server pb.KaryawanServiceServer
	{
		...
		service := svc.NewKaryawan(dbReadWriter)
		endpoint := ep.NewKaryawanEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCKaryawanServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterKaryawanServiceServer(grpcServer, server)
