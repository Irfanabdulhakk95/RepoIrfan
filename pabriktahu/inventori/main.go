...
import (
	...
	ep "pabriktahu/inventori/endpoint"
	pb "pabriktahu/inventori/grpc"
	svc "pabriktahu/inventori/server"

	cfg "pabriktahu/util/config"
	run "pabriktahu/util/grpc"
...
func main() {
	...
	var server pb.InventoriServiceServer
	{
		...
		service := svc.NewInventori(dbReadWriter)
		endpoint := ep.NewInventoriEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCInventoriServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterInventoriServiceServer(grpcServer, server)
